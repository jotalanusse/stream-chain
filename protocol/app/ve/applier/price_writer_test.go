...
func TestPriceWriter(t *testing.T) {
	...
	t.Run("valid ve set with on absent block id flag", func(t *testing.T) {
		...
		vote3, err := vetesting.CreateNonSignedAbsentExtendedVoteInfo(
			vetesting.NewDefaultSignedVeInfo(
				constants.CarlConsAddress,
				prices1,
			),
		)
		require.NoError(t, err)

		vote4, err := vetesting.CreateSignedExtendedVoteInfo(
			vetesting.NewDefaultSignedVeInfo(
				constants.DaveConsAddress,
				prices1,
			),
		)
		require.NoError(t, err)
		...
		_, extCommitInfoBz, err := vetesting.CreateExtendedCommitInfo(
			[]cometabci.ExtendedVoteInfo{vote1, vote2, vote3, vote4},
		)
		require.NoError(t, err)
