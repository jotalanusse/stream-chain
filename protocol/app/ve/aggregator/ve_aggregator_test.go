...
var (
	...
	emptyExtraPriceInfo = map[string]*big.Int{}
)
...
func SetupTest(t *testing.T, vals []string) (sdk.Context, veaggregator.VoteAggregator) {
	...
	aggregateFn := voteweighted.MedianWithMarkAndFundingRate(
		ctx.Logger(),
		mCCVStore,
...
func TestVEAggregator(t *testing.T) {
	t.Run("no daemon data", func(t *testing.T) {
		...
		prices, err := handler.AggregateDaemonVEIntoFinalPrices(ctx, votes, emptyExtraPriceInfo, emptyExtraPriceInfo, emptyExtraPriceInfo)
		require.NoError(t, err)
