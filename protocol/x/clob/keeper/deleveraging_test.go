				...
				mockIndexerEventManager.On("AddTxnEvent",
					...
					indexer_manager.GetBytes(
						indexerevents.NewDeleveragingEvent(
							...
							satypes.BaseQuantums(dtypes.NewIntFromBigInt(bankruptcyPriceQuoteQuantums)),
					...
					indexer_manager.GetBytes(
						indexerevents.NewDeleveragingEvent(
							...
							*tc.offsettingSubaccount.GetId(),
							uint32(0),
							satypes.BaseQuantums(dtypes.NewIntFromBigInt(new(big.Int).Abs(tc.deltaQuantums))),
							satypes.BaseQuantums(dtypes.NewIntFromBigInt(bankruptcyPriceQuoteQuantums)),
							tc.deltaQuantums.Sign() > 0,
							false,
						),
					...
					indexer_manager.GetBytes(
						indexerevents.NewDeleveragingEvent(
							...
							*tc.offsettingSubaccount.GetId(),
							uint32(0),
							satypes.BaseQuantums(dtypes.NewIntFromBigInt(new(big.Int).Abs(tc.deltaQuantums))),
							satypes.BaseQuantums(dtypes.NewIntFromBigInt(fillPriceQuoteQuantums)),
							tc.deltaQuantums.Sign() > 0,
							false,
						),
					),
