	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.subaccountModuleAccBalance > 0 {
				err := bank_testutil.FundAccount(
					...
					sdk.Coins{
						sdk.NewCoin(
							constants.TDai.Denom,
							sdkmath.NewIntFromBigInt(big.NewInt(tc.subaccountModuleAccBalance)),
						),
					},
					*bankKeeper,
				)
			...
			require.Equal(
				...
				sdk.NewCoin(
					constants.TDai.Denom,
					sdkmath.NewIntFromBigInt(big.NewInt(tc.expectedSubaccountsModuleAccBalance)),
				),
				subaccountsModuleAccBalance,
			)
			...
			require.Equal(t,
				sdk.NewCoin(
					constants.TDai.Denom,
					sdkmath.NewIntFromBigInt(big.NewInt(tc.expectedInsuranceFundBalance)),
				),
				toModuleBalance,
			)
		})
